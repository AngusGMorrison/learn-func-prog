defmodule DepthNavigator do
  ...
  defp print_and_navigate(dir, true, current_depth) do
    IO.puts dir
    child_dirs = File.ls!(dir)
    go_through(expand_dirs(child_dirs, dir), current_depth + 1)
  end

  defp expand_dirs([], _relative_to), do: []
  defp expand_dirs([head | tail], relative_to) do
    expanded_dir = Path.expand(head, relative_to)
    [expanded_dir | expand_dirs(tail, relative_to)]
  end
